...
/// When `use_jianpu` is true, uses jianpu layout so the cursor matches
/// jianpu rendering and does not land on key or time signature symbols.
pub fn generate_playback_map(
    ...
    let (measure_positions, system_positions) =
        compute_measure_positions(score, page_width, staff_indices_1based, use_jianpu);
