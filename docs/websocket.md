...
The SoloBand Ultra iOS and Android apps join the cloud choir by room name and password. The WebSocket base URL is set at **build time**:

- **iOS**: Set `CHOIR_WS_BASE_URL` in `Info.plist` (e.g. `https://your-choir-server.example.com`). The app connects to `wss://your-choir-server.example.com/ws` with the Kinde Bearer token and sends `join` with room and password. If the room does not exist, the first joiner creates it.
- **Android**: Set the same URL in `build.gradle.kts` via `buildConfigField("String", "CHOIR_WS_BASE_URL", "\"https://your-choir-server.example.com\"")` (or a product flavor). Empty string shows a "not configured" message in the Choir sheet.
